...
export default function ReportExportModal({ open, onClose }: ReportExportModalProps) {
  ...
  const reports = [
    ...
    },
    {
      ...
      name: "Tempo de Atendimento (Operadores)",
      description: "Métricas de TMA (Tempo Médio de Atendimento) por atendente em todos os funis.",
      available: false
    }
  ];
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6" onClick={onClose}>
      ...
      <div 
        className="relative flex flex-col w-full max-w-4xl max-h-[90vh] bg-zinc-950 border border-white/10 rounded-2xl shadow-2xl overflow-hidden"
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/5 bg-white/5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-violet-600/20 flex items-center justify-center border border-violet-500/30 text-violet-400">
              <FileSpreadsheet className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-white tracking-wide">Exportação de Relatórios</h2>
              <p className="text-xs text-zinc-400 mt-0.5">Baixe dados consolidados do Fagner em formato XLSX.</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
          ...
          <section>
            <h3 className="text-sm font-semibold text-zinc-300 mb-4 flex items-center gap-2">
              <Search className="w-4 h-4 text-violet-400" /> Relatórios Disponíveis
            </h3>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <h4 className="text-sm font-bold text-zinc-100">{r.name}</h4>
                        {!r.available && <span className="text-[10px] px-2 py-0.5 rounded-full bg-zinc-800 text-zinc-400 font-semibold border border-white/10">Em breve</span>}
                        {r.available && <span className="text-[10px] px-2 py-0.5 rounded-full bg-green-500/20 text-green-400 font-semibold border border-green-500/30">Disponível XLSX</span>}
                      </div>
                    ...
                    {r.available && (
                      <div className="flex flex-col sm:flex-row items-center gap-3">
                        <div className="flex items-center gap-2 bg-black/40 p-1.5 rounded-lg border border-white/5">
                          <div className="flex items-center gap-2 px-2">
                            ...
                            <input 
                              ...
                              className="bg-transparent text-xs text-zinc-200 outline-none w-28 placeholder-zinc-700" 
                        ...
                        <button
                          ...
                          className="px-4 py-2 bg-violet-600 hover:bg-violet-500 active:bg-violet-700 text-white rounded-lg text-sm font-bold shadow-lg shadow-violet-900/50 transition-all flex items-center gap-2 disabled:opacity-50"
                        >
                          {downloading ? (
          ...
          <section>
            <h3 className="text-sm font-semibold text-zinc-300 mb-4 flex items-center gap-2">
              <History className="w-4 h-4 text-orange-400" /> Registro de Auditoria & Downloads
            </h3>
            ...
            <div className="border border-white/10 rounded-xl overflow-hidden bg-black/40">
              {loadingLogs ? (
                ...
                <div className="p-8 text-center">
                  <p className="text-sm text-zinc-500">Nenhum relatório foi baixado ainda.</p>
                </div>
              ) : (
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-white/5 text-[10px] uppercase text-zinc-400 font-bold tracking-wider">
                      <th className="p-3 border-b border-white/5">Data/Hora</th>
                      <th className="p-3 border-b border-white/5">Relatório</th>
                      <th className="p-3 border-b border-white/5">Filtros Usados</th>
                      <th className="p-3 border-b border-white/5 whitespace-nowrap">Baixado Por</th>
                    </tr>
                  </thead>
                  <tbody>
                    {logs.map((log) => (
                      <tr key={log.id} className="text-xs text-zinc-300 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                        ...
                        <td className="p-3">
                          <span className="px-2 py-1 bg-white/5 border border-white/10 rounded text-zinc-300 text-[10px] font-bold">
                            {log.downloadedBy}
                          </span>
                        </td>
                      </tr>
